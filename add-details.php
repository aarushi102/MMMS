<?php
    ...
    if(isset($_POST['add_details'])){
      //  if(!empty($_POST['course_no']) && !empty($_POST['y1']) && !empty($_POST['cc1']) && !empty($_POST['c1']) && !empty($_POST['p1']) && !empty($_POST['cn1']) && !empty($_POST['d1']) && !empty($_POST['de1']) && !empty($_POST['sem']) && !empty($_POST['f1'])){
            ...
            $query= "INSERT INTO `mp_course_obj_outcomes` (`y1`, `cc1`, `c1`, `p1`, `cn1`, `d1`, `de1`, `sem`, `f1`) VALUES ('$_POST[y1]', '$_POST[cc1]', '$_POST[c1]', '$_POST[p1]', '$_POST[cn1]', '$_POST[d1]', '$_POST[de1]', '$_POST[sem]', '$_POST[f1]')";
...
<script type="text/javascript">
	window.location.href = "MP_Course_Objectives_Outcome.php";
</script>