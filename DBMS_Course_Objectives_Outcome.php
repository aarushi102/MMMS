...
<html lang="en">
  ...
  <form action="" method=""> 
    ...
    <form action="dbms-add-details.php" method="POST">
    ...
    <h2 align="left">Course Objectives:</h2>
      ...
      <?php
					$query = "select * from dbms_course_obj_outcomes_2";
					$query_run = mysqli_query($connection,$query);
					while ($row = mysqli_fetch_assoc($query_run)) 
					{
						?>
						<center>
						<table style=class="table table-dark table-hover align="left" border="1" height="70px" width="80%" cellspacing="0">
							<tr>
								<td id="td"><?php echo $row['sr.no']?></td>
								<td id="td"><?php echo $row['statement']?></td>
							</tr>
						</table>
						</center>
						<?php
...
<table class="table table-dark table-hover width="100%" align="center" height="60px">
  <tr>
  <td align="center">
    <h4>To add a New Objective...</h4>
    <input type="submit" name="add_new_objective" value="Add Objective">
  </td>
</tr>
</table>
<?php
    if(isset($_POST['add_new_objective'])){
        ?>
						<center>
						<table align="left" border="1" height="70px" width="80%" cellspacing="0">
							<tr>
								<td><input type="text" name="sr.no"></td>
...
<br><br>
      ...
      <!-- //  if(!empty($_POST['course_no']) && !empty($_POST['y1']) && !empty($_POST['cc1']) && !empty($_POST['c1']) && !empty($_POST['p1']) && !empty($_POST['cn1']) && !empty($_POST['d1']) && !empty($_POST['de1']) && !empty($_POST['sem']) && !empty($_POST['f1'])){
            ...
            <!-- <?php
    //         $query= "INSERT INTO `course_obj_outcomes_2` (`statement`) VALUES ('$_POST[statement]')";
    //         $query_run = mysqli_query($connection,$query);
    //         while ($row = mysqli_fetch_assoc($query_run)) 
	// 				{
